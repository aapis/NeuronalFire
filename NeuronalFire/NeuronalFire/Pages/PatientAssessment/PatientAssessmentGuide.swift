...
struct PatientAssessmentGuide: View {
    ...
    typealias PAType = PatientAssessment.ScenarioType
...
//                                ),
    ...
    struct Scenario: Identifiable, Equatable {
        ...
        static func == (lhs: PatientAssessment.Scenario, rhs: PatientAssessment.Scenario) -> Bool {
                            ...
                            ForEach(section.requirements) { req in
                                VStack(alignment: .leading, spacing: 1) {
                                    ...
                                    if req.notes != nil {
                                        ...
                                        .font(.caption)
                                        .foregroundStyle(.gray)
                                        .background(current.requirement == req.id ? .blue : req.importance == .critical ? .yellow : .gray.opacity(0.3))
                                    }
                                }
                            }
                            .opacity(self.completed ? 0.5 : 1)
